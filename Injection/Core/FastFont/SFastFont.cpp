...
void CSFastFont::DrawText(LPSTR str, int x, int y, DWORD FontColor, int Arrange, SIZE *pSize)
{
	...
	while (c[0] != _T('\0'))
	{
		...
		else
		{
			...
			if (size.cy < yy + tempsize.cy - 1)
				size.cy = yy + tempsize.cy - 1;
		}
	}
...
void CSFastFont::Flush(void)
{
	...
	if (m_pCreateOrderInfo)
	{
		...
		if (hr == D3D_OK)
		{
			while (pNextOrder)
			{
				...
				{
					memset(((BYTE*)ddsd.lpSurface) + ddsd.lPitch * (yy + pNextOrder->v) + pNextOrder->u * 2, 0x00, m_MaxFontWidth * 2);
				}

				m_pFastFont->SetBltStatus(ddsd.lpSurface, ddsd.lPitch, 16, m_ImageMode, SFontBlt16Black);
				m_pFastFont->BltFontData(pNextOrder->OriginalKey, pNextOrder->u + 1, pNextOrder->v    , &tempsize);
				m_pFastFont->BltFontData(pNextOrder->OriginalKey, pNextOrder->u    , pNextOrder->v + 1, &tempsize);
				m_pFastFont->BltFontData(pNextOrder->OriginalKey, pNextOrder->u + 2, pNextOrder->v + 1, &tempsize);
				m_pFastFont->BltFontData(pNextOrder->OriginalKey, pNextOrder->u + 1, pNextOrder->v + 2, &tempsize);
				m_pFastFont->SetBltStatus(ddsd.lpSurface, ddsd.lPitch, 16, m_ImageMode, SFontBlt16);
				m_pFastFont->BltFontData(pNextOrder->OriginalKey, pNextOrder->u + 1, pNextOrder->v + 1, &tempsize);
	...
	if (m_SSF_Vertex_index)
	{
		...
		m_d3ddev->DrawPrimitive(D3DPT_TRIANGLELIST, D3DFVF_TLVERTEX, m_SFF_Vertex, m_SSF_Vertex_index, 0);
