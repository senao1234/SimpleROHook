...
CFastFont::StOutlineFontData *CFastFont::GetFontData(int code, SIZE *pSize)
{
	...
	if (!pData)
	{
		...
		imagesize = GetGlyphOutline(m_hDC, code, m_OutLineFormat, &GM, m_DefaultBufferSize, m_pDefaultBuffer, &Mat);
		...
		if (m_BltAAMode)
		{
			...
			for (yy = 0; yy < iBmp_h + 1; yy++)
			{
				...
				for (xx = 0; xx < iBmp_w + 1; xx++)
				{
					...
					if (m_Data2PicFunc)
						m_Data2PicFunc(m_DistPtr + m_DistPitch * (y + yy + iOfs_y) + (m_DistBits / 8) * (x + xx + iOfs_x), Alpha);
				}
			}
